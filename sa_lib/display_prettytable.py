...
class display(object):
    def __init__(self):
        ...
        self.x.field_names = [" Al.No ", " Wall Time ", " Stamp Time ", " State "]
        self.t_no  = 0

        self.CUR_TZ = pytz.timezone(settings.TZ)
        ...
        if settings.logging:
            self.logfile_name = 'sa_log_'+ datetime.now(tz=self.CUR_TZ).strftime("%Y%m%d_%H%M") + '.log'
    ...
    def add_t(self, al, state, timestamp):
        if al is None:
            ...
            al_no = al['alarm_no']
        wall_time = datetime.now(tz=self.CUR_TZ)

        t_fmt = '%Y/%m/%d %H:%M:%S'
        self.x.add_row([al_no, wall_time.strftime(t_fmt), timestamp.strftime(t_fmt), state])
        self.t_no += 1
        ...
        if settings.logging:
            with open(self.logfile_name, 'a') as lf:
                lf.write(str(al_no) +'\t'+ wall_time.strftime(t_fmt) +'\t'+ timestamp.strftime(t_fmt) +'\t'+ state +'\n')
