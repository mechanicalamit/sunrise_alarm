...
class RPiSunrise(object):
    ...
    def is_ready(self, indate):
        ...
        if self.al['enabled'] and self.al['dow'][indate.weekday()] and ( 0 <= (hhmm - self.al['ready_t']) < 100 ):
            return True
        return False

    def can_ring(self, indate):
        hhmm = indate.hour * 100 + indate.minute
        if self.al['enabled'] and self.al['dow'][indate.weekday()] and ( 0 <= (hhmm - self.al['run_t']) < 100 ):
            self.ringingal = al
            return True
        return False
